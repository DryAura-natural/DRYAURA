  ...
  />
    ...
    <div className="flex flex-col sm:flex-row justify-between sm:items-center">
      ...
      <div className="flex items-center justify-center space-x-2 mt-2 sm:mt-0 sm:ml-4 border border-white/80 p-1 rounded-full backdrop-blur-md bg-transparent text-white w-28 shadow-lg ">
  <button
    onClick={onDecrement}
    className="bg-white/20 p-2 rounded-full text-sm hover:bg-white/30 transition-all duration-200 text-black shadow-sm border border-black"
    aria-label="Decrease quantity"
  >
    <Minus size={14} color="black"/>
  </button>
  <span className="text-lg font-semibold w-6 text-center text-black">{data.quantity}</span>
  <button
    onClick={onIncrement}
    className="bg-white/20 p-2 rounded-full text-sm hover:bg-white/30 transition-all duration-200 text-black shadow-sm border border-black"
