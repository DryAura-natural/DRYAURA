...
const Footer = () => {
  return (
    <footer className="w-full text-white mt-auto relative rounded-t-3xl bg-[#3D1D1D] pt-12 pb-16 overflow-hidden">
      ...
      <div className="bg-black/10 text-white border-t border-white/10 mt-0">
        <div className="max-w-screen-xl mx-auto px-4 py-5">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 items-center justify-between">
            {/* Left Column - Important Policy Links */}
            <div className="flex flex-col items-start">
              <h4 className="text-sm font-semibold text-white/80 mb-4">Policies</h4>
              <div className="space-y-3 w-full">
                {importent.map((item) => (
                  <a
                    key={item.label}
                    href={item.link}
                    className="text-xs sm:text-sm text-white/70 hover:text-white hover:underline transition-all flex items-center space-x-2"
                  >
                    {item.icon}
              ...
              <div className="flex flex-wrap gap-4 items-center">
                {[
                  ...
                  <a
                    ...
                    aria-label={label}
                    className="text-2xl sm:text-3xl text-white/70 hover:text-white hover:scale-110 transition-all ease-in-out"
                  >
                    <Icon />
                  </a>
                ))}
